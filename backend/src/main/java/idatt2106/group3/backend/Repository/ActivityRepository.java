...
public interface ActivityRepository extends JpaRepository<Activity, Long>
{
    ...
    @Query(value = "SELECT * FROM activity WHERE activity.title LIKE ?1 OR activity.description LIKE ?1 ORDER BY activity.start_time LIMIT ?2", nativeQuery = true)
    ...
    @Query(value = "SELECT * FROM activity WHERE (activity.title LIKE ?1 OR activity.description LIKE ?1) AND activity.difficulty=?3 ORDER BY activity.start_time LIMIT ?2", nativeQuery = true)
    public List<Activity> findActivitiesOnDateWithFilter(String searchQuery, Integer amount, Integer difficulty);

    @Query(value = "SELECT activity.* FROM activity LEFT JOIN user_activity ON (activity.activity_id = user_activity.activity_id ) WHERE activity.title LIKE ?1 OR activity.description LIKE ?1 GROUP BY activity.activity_id ORDER BY COUNT(user_activity.activity_id) DESC LIMIT ?2", nativeQuery = true)
    public List<Activity> findActivitiesOnAmountWithoutFilter(String searchQuery, Integer amount);

    @Query(value = "SELECT activity.* FROM activity LEFT JOIN user_activity ON (activity.activity_id = user_activity.activity_id ) WHERE (activity.title LIKE ?1 OR activity.description LIKE ?1) AND activity.difficulty = ?3 GROUP BY activity.activity_id ORDER BY COUNT(user_activity.activity_id) DESC LIMIT ?2", nativeQuery = true)
    public List<Activity> findActivitiesOnAmountWithFilter(String searchQuery, Integer amount, Integer difficulty);

    @Query(value = "SELECT activity.* FROM activity WHERE activity.title LIKE ?1 OR activity.description LIKE ?1 ORDER BY SQRT(POW((?3-longitude),2)+POW((?4-latitude),2)) LIMIT ?2", nativeQuery = true)
    ...
    @Query(value = "SELECT activity.* FROM activity WHERE (activity.title LIKE ?1 OR activity.description LIKE ?1) AND activity.difficulty = ?5 ORDER BY SQRT(POW((?3-longitude),2)+POW((?4-latitude),2)) LIMIT ?2", nativeQuery = true)
